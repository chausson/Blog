...
这个章节是介绍一些塑造 LLVM[](http://www.aosabook.org/en/llvm.html#footnote-1) 的一些设计决策，LLVM是应用在Unix系统中兼容现有工具作的代表性设计，作为一个综合工程项目它承载和开发一套严谨的低层工具链组件(例如: 汇编器、编译器、调试器等)。LLVM这个名称曾经是一个缩略词，但是现在只代表一个综合工程项目的品牌名称。当LLVM证明一些独特的功能时，同时它服务一些很棒的工具（如 Clang 编译器[](http://www.aosabook.org/en/llvm.html#footnote-2),一个C/ c++ /Objective-C编译器，它比GCC编译器更加好用和高效），LLVM最主要区别在于它的内部架构与其他编译器的不同。
